...
def test_full_pipeline_flow(caplog):
    ...
    with caplog.at_level(logging.INFO), patch(
        "ingestion.fetch_stocks.yf.download", return_value=_mock_yf_df()
    ), patch("ingestion.fetch_stocks.boto3.client", return_value=mock_s3), patch(
        "ingestion.anomaly_detector.boto3.client", return_value=mock_s3
    ), patch(
        "ingestion.price_predictor.boto3.client", return_value=mock_s3
    ), patch(
        "ingestion.market_insights.boto3.client", return_value=mock_s3
    ), patch(
        "ingestion.market_insights.openai.OpenAI", return_value=mock_oi
    ):
